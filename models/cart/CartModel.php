...
class Cart implements Billable, IAggregater
{
    ...
    static public function get_all_carts_by_user_id($user_id): array
    {
        ...
        $cartDataResult = $conn->run_select_query($query);
        ...
        if ($cartDataResult) {
            ...
            while ($cartDataIterator->hasNext()) {
                ...
                $cartItemsResult = $conn->run_select_query($itemsQuery);
    ...
    static public function get_current_cart_by_user_id($user_id): ?Cart
    {
        ...
        $result = $conn->run_select_query($query)->fetch_assoc();

        if ($result) {
            $cart = new Cart($result);

            // Fetch cart items
            $cart_items = $conn->run_select_query("
    ...
    public static function add_new_cart(int $user_id): void
    {
        global $configs;
        global $conn;
        $conn->run_query("INSERT INTO $configs->DB_NAME.$configs->DB_CARTS_TABLE (user_id) VALUES ($user_id)");
    }
    ...
    public function add_item_to_cart($item_id): bool
    {
        ...
        if ($conn->run_select_query("SELECT * FROM $configs->DB_NAME.$configs->DB_CART_ITEMS_TABLE WHERE `cart_id` = $cart_id AND `item_id` = $item_id")->num_rows > 0) {
            // Increment if exists
            return $conn->run_query("UPDATE $configs->DB_NAME.$configs->DB_CART_ITEMS_TABLE SET `quantity` = `quantity` + 1 WHERE `cart_id` = $cart_id AND `item_id` = $item_id");
        } else {
            // Otherwise add new entry
            return $conn->run_query("INSERT INTO $configs->DB_NAME.$configs->DB_CART_ITEMS_TABLE (`cart_id`, `item_id`, `quantity`) VALUES ($cart_id, $item_id, 1)");
        }
    }
    ...
    public function remove_item_from_cart($item_id): bool
    {
        ...
        $result = $conn->run_select_query("SELECT quantity FROM $configs->DB_NAME.$configs->DB_CART_ITEMS_TABLE WHERE `cart_id` = $cart_id AND `item_id` = $item_id");
        if ($result->num_rows > 0) {
            ...
            if ($currentQuantity > 1) {
                ...
                $success = $conn->run_query("UPDATE $configs->DB_NAME.$configs->DB_CART_ITEMS_TABLE SET `quantity` = `quantity` - 1 WHERE `cart_id` = $cart_id AND `item_id` = $item_id");
                if (!$success) {
                ...
                $success = $conn->run_query("DELETE FROM $configs->DB_NAME.$configs->DB_CART_ITEMS_TABLE WHERE `cart_id` = $cart_id AND `item_id` = $item_id");
                if (!$success) {
    ...
    static public function delete_cart_by_user_id(int $user_id): bool
    {
        ...
        $cart_ids = $conn->run_select_query("SELECT id FROM $configs->DB_NAME.$configs->DB_CARTS_TABLE WHERE `user_id` = $user_id")->fetch_all(MYSQLI_ASSOC);
        ...
        while ($cartIterator->hasNext()) {
            ...
            if (!$conn->run_query("DELETE FROM $configs->DB_NAME.$configs->DB_CARTS_TABLE WHERE `id` = $cart_id")) {
