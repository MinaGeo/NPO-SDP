...
class Event
{
    ...
    {
        $rows = run_select_query("SELECT * FROM `NPO`.`event` WHERE id = ?", [$id]);
    ...
    {
        $events = [];
        $rows = run_select_query("SELECT * FROM `event`")->fetch_all(MYSQLI_ASSOC);
    ...
    static public function add_event(string $name, string $description, string $location, string $type, string $date): bool
    {
        ...
        $checkEvent = run_select_query("SELECT * FROM `NPO`.`event` WHERE `name` = ?", [$name], true);
        
        // Use num_rows to check if there are no results
        if ($checkEvent && $checkEvent->num_rows == 0) {  
            // Insert the new event
            return run_query("INSERT INTO `NPO`.`event` (`name`, `description`, `location`, `type`, `date`) VALUES (?, ?, ?, ?, ?)", 
    ...
    static public function delete_event(int $id): bool
    {
        ...
        $result = run_select_query("SELECT * FROM `NPO`.`event` WHERE `id` = $id");

        if ($result && $result->num_rows > 0) {
            // Remove the event
            $success = run_query("DELETE FROM `NPO`.`event` WHERE `id` = $id");
